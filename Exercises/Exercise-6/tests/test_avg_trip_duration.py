from main import avg_trip_duration_per_day
import zipfile
import csv
import io
import pytest
...
def test_avg_trip_duration_per_day(tmp_path,spark):
    #Arrange
    ...
    #Act
    result = avg_trip_duration_per_day(str(output_folder),df)
    ...
    output_dir = output_folder / 'avg_trip_duration.csv'
    assert output_dir.exists()

    #validate the logic
    written_df = spark.read.csv(str(output_dir),header=True,inferSchema=True)
    expected_data = [
        Row(trip_date="2019-10-01",avg_trip_duration=599.0)
    ]
