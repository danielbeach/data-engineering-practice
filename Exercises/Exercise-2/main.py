...
def main():
    ...
    localFilename = matching_url.split('/')[-1]
    save_file = os.getcwd()
    flag = str(save_file).split('/')
    if "Exercises" not in flag:
        filePath = os.path.join(save_file, "Exercises", "Exercise-2", localFilename)
    else :
        filePath = os.path.join(save_file, "Exercise-2", localFilename)
