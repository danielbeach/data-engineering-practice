...
The aim of this repository is to help you develop and 
...
run the `docker build` command, which will be listed in
...
tests your ability to perform web scraping, build uris, download files, and use Pandas to
...
focuses more on file types `json` and `csv`, and working with them in `Python`.
...
Taking a page out of the previous exercise, this one is focused on using a few of the
more common built-in PySpark functions `pyspark.sql.functions` and applying their
...
will require an understanding of the functions and documentation of DuckDB.
...
be given an existing dataset in CSV format, as well as an existing pipeline. There is a data quality issue , 
